...
from ..models import PointVente, SessionCaisse, SessionPlanning, Vente
...
def _get_context_data():
    ...
    sessions_ouvertes = SessionCaisse.objects.filter(statut='OUVERTE').select_related('point_vente', 'caissier_ouverture').order_by('-date_ouverture')
    sessions_attente = SessionCaisse.objects.filter(statut='FERMEE', fonds_collectes=False).select_related('point_vente', 'caissier_ouverture', 'caissier_fermeture').order_by('-date_fermeture')
    sessions_collectees_ajd = SessionCaisse.objects.filter(date_collecte__date=today, fonds_collectes=True).select_related('point_vente', 'ferme_par_raf', 'transfert_caisse').order_by('-date_collecte')
    ...
    plannings_ajd = SessionPlanning.objects.filter(date=today).exclude(statut='ANNULE').select_related('point_vente', 'employe').order_by('heure_debut')
    ...
    return {
        ...
        'sessions_collectees_ajd': sessions_collectees_ajd,
        'transferts_ajd': transferts_ajd,
...
def raf_dashboard_data_api(request):
    """API JSON pour le dashboard dynamique Alpine.js"""
    if not _is_raf(request.user):
    ...
    sessions_attente = SessionCaisse.objects.filter(statut='FERMEE', fonds_collectes=False)
    sessions_collectees_ajd = SessionCaisse.objects.filter(date_collecte__date=today, fonds_collectes=True)
    transferts_ajd = TransfertCaisse.objects.filter(date__date=today)
    plannings_ajd = SessionPlanning.objects.filter(date=today).exclude(statut='ANNULE')
    ...
    ouvertes_data = []
    for s in sessions_ouvertes.select_related('point_vente', 'caissier_ouverture').order_by('-date_ouverture'):
        ouvertes_data.append({
            'id': s.id,
            'point_vente': s.point_vente.nom if s.point_vente else 'N/A',
            'caissier': s.caissier_ouverture.nom_complet if s.caissier_ouverture else 'N/A',
            'date_ouverture': s.date_ouverture.strftime('%d/%m/%Y %H:%M'),
            'duree': s.date_ouverture.strftime('%H:%M') if s.date_ouverture else '-',
            'nb_ventes': s.nombre_ventes,
    ...
    attente_data = []
    for s in sessions_attente.select_related('point_vente', 'caissier_ouverture', 'caissier_fermeture').order_by('-date_fermeture'):
        attente_data.append({
            'id': s.id,
            'point_vente': s.point_vente.nom if s.point_vente else 'N/A',
            'caissier': s.caissier_ouverture.nom_complet if s.caissier_ouverture else 'N/A',
            ...
            'nb_ventes': s.nombre_ventes,
            'total_ventes': float(s.total_ventes),
            'solde_attendu': float(s.solde_initial + s.total_ventes),
        })

    collectees_data = []
    for s in sessions_collectees_ajd.select_related('point_vente', 'ferme_par_raf', 'transfert_caisse').order_by('-date_collecte'):
        collectees_data.append({
            'id': s.id,
            'point_vente': s.point_vente.nom if s.point_vente else 'N/A',
            'raf': s.ferme_par_raf.nom_complet if s.ferme_par_raf else 'N/A',
            'date_collecte': s.date_collecte.strftime('%H:%M') if s.date_collecte else 'N/A',
            'montant': float(s.depot or 0),
            'reference': s.transfert_caisse.reference if s.transfert_caisse else '-',
        })
    ...
    for t in transferts_ajd.select_related('source', 'destination', 'valide_par').order_by('-date'):
        transferts_data.append({
            ...
            'heure': t.date.strftime('%H:%M') if t.date else '-',
            'source': t.source.nom,
    ...
    plannings_data = []
    for p in plannings_ajd.select_related('point_vente', 'employe').order_by('heure_debut'):
        plannings_data.append({
            'id': p.id,
            'point_vente': p.point_vente.nom,
            'employe': p.employe.nom_complet,
            'horaire': f"{p.heure_debut.strftime('%H:%M')} - {p.heure_fin.strftime('%H:%M')}",
            'statut': p.statut,
    ...
    for d in RafCaisseService.get_demandes_ouverture():
        demandes_data.append({
            'point_vente_id': d['point_vente'].id,
            'caisse_id': d['caisse'].id,
            'planning_id': d['planning'].id,
            'point_vente': d['point_vente'].nom,
            'employe': d['employe'].nom_complet,
            'planning': f"{d['planning'].heure_debut.strftime('%H:%M')} - {d['planning'].heure_fin.strftime('%H:%M')}",
    ...
    return JsonResponse({
        'success': True,
        'stats': stats,
        'ouvertes': ouvertes_data,
        'attente': attente_data,
        'collectees': collectees_data,
        'transferts': transferts_data,
